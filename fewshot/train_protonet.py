...
if __name__ == '__main__':
	...
	parser.add_argument('--model_type', type=str, default='ConvNet',
						choices=['ConvNet', 'ResNet', 'AmdimNet', 'SphConvNet', 'SphResNet'])
	...
	parser.add_argument('--metric', default='euclidean', choices=['euclidean', 'cos'])
...
#	set_gpu(args.gpu)
	...
	if args.model_type.startswith('Sph'):
		model_type = args.model_type[3:]
	else:
	...
	if args.init_weights is not None:
		model_detail = torch.load(args.init_weights, map_location=device)
	...
	for epoch in range(1, args.max_epoch + 1):
		...
		for i, batch in enumerate(train_loader, 1):
			...
			print('\repoch {}, train {}/{}, loss={:.4f} acc={:.4f} |'
				  .format(epoch, i, len(train_loader), loss.item(), acc), end=' ')
		...
		print('best epoch {}, best val acc={:.4f} |'.format(trlog['max_acc_epoch'], trlog['max_acc']), end=' ')
		with torch.no_grad():
		...
		print('epoch {}, val, loss={:.4f} acc={:.4f} |'.format(epoch, vl, va), end=' ')
		...
		print('ETA:{}/{}'.format(timer.measure(), timer.measure(epoch / args.max_epoch)), end=' ')
	writer.close()
	print()

	# Test Phase
	trlog = torch.load(osp.join(args.save_path, 'trlog'), map_location=device)
	...
	model.load_state_dict(torch.load(osp.join(args.save_path, 'max_acc' + '.pth'), map_location=device)['params'])
	model.eval()
	...
	with torch.no_grad():
		for i, batch in enumerate(loader, 1):
			...
			print('\rbatch {}: {:.2f}({:.2f})'.format(i, ave_acc.item() * 100, acc * 100), end=' ')
	print()
