...
parser.add_argument('--export-dir', type=str, default='./experiments_results_baseline')

args = parser.parse_args()

export_prefix_base = 'experiment.scoring_comparison.{}'.format(args.model_tag)
...
if not SKIP_PPL:
    ...
    logging.info('##########################################')
    export_prefix = export_prefix_base + '.ppl_baseline'
    no_inference = False
    for _model, _max_length, _batch in models:
        scorer = alm.RelationScorer(model=_model, max_length=_max_length,
									cache_dir=args.cache_dir, gpu_id=args.gpu_id)
